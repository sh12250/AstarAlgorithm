...
public class PathFinder : GSingleton<PathFinder>
{
    ...
    {
        // Astar 알고리즘을 사용하기 위해서 패스 리스트를 초기화한다
        ...
        while (loopIdx < 10)
        {
            ...
            {
                // { 도착하지 않았다면 현재 타일을 기준으로 4방향의 노드를 찾아온다
                ...
                // 이 때, open list가 비어 있다면 더 이상 탐색할 수 있는 길이 
                // 존재하지 않는 것이다
    ...
    private void Add_AstarOpenList(AstarNode targetTerrain_, AstarNode prevNode_ = default)
    {
        ...
        else
        {
            ...
            {
                // 타겟 노드의 코스트가 더 작은 경우에는 open list의 노드와 교체한다
